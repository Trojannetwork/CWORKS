/* === Beams Effect — Vanilla Three.js (ES Module) ===
   Creates animated noise-displaced vertical planes in the hero section.
   Loads Three.js from CDN import map. */

import * as THREE from 'three';

const hero = document.getElementById('hero');
const canvasContainer = document.getElementById('beams-canvas');
if (!hero || !canvasContainer) throw new Error('Hero or beams canvas not found');
...
/* ---- Geometry: stacked vertical planes ---- */
...
beamGroup.rotation.z = degToRad(cfg.rotation);
scene.add(beamGroup);

const material = createBeamMaterial();
const geometry = createStackedPlanesGeometry(cfg.beamNumber, cfg.beamWidth, cfg.beamHeight, 0, 100);
const mesh = new THREE.Mesh(geometry, material);
beamGroup.add(mesh);

const dirLight = new THREE.DirectionalLight(cfg.lightColor, 1);
